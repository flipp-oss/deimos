...
RSpec.describe Deimos::SchemaBackends::ProtoSchemaRegistry do
  ...
  let(:proto_turf) { instance_double(ProtoTurf) }
  let(:key_proto_turf) { instance_double(ProtoTurf) }

  let(:backend) { described_class.new(schema: 'sample.v1.SampleMessage') }

  before(:each) do
    allow(described_class).to receive_messages(proto_turf: proto_turf, key_proto_turf: key_proto_turf)
  end
  ...
  describe 'payload encoding and decoding' do
    it 'should encode and decode payloads correctly' do
      ...
      expect(proto_turf).to have_received(:encode).with(payload, subject: 'topic-value')
    ...
    it 'should encode hash payloads by converting to protobuf message' do
      ...
      allow(proto_turf).to receive(:encode).and_return('encoded-payload')

      backend.encode(hash_payload, topic: 'topic')

      # Should have converted hash to protobuf message before encoding
      expect(proto_turf).to have_received(:encode) do |arg, **kwargs|
  ...
  describe 'key encoding with key_config field (auto-generated JSON schema)' do
    it 'should encode a simple field key using JSON schema' do
      ...
      allow(key_proto_turf).to receive(:encode).and_return('encoded-key')

      # encode_key is called from encode_proto_key with the hash
      encoded = backend.encode_key('str', 'test-key', topic: 'my-topic')

      expect(key_proto_turf).to have_received(:encode).with(
        key_hash,
    ...
    it 'should decode a JSON schema key' do
      ...
      allow(key_proto_turf).to receive(:decode).and_return(decoded_json)

      result = backend.decode_key('encoded-key', 'str')

      expect(key_proto_turf).to have_received(:decode).with('encoded-key')
  ...
  describe 'key encoding with key_config schema (separate Protobuf schema)' do
    ...
    let(:key_proto_turf_for_test) { instance_double(ProtoTurf) }
    let(:proto_turf_for_test) { instance_double(ProtoTurf) }

    before(:each) do
      # For key_backend tests, mock both proto_turf instances
      allow(described_class).to receive_messages(proto_turf: proto_turf_for_test,
                                                 key_proto_turf: key_proto_turf_for_test)
    end

    it 'should encode a protobuf key message using the key schema' do
      key_msg = Sample::V1::SampleMessageKey.new(str: 'test-key')
      # Since subject ends with '-key', encode_payload uses key_proto_turf
      allow(key_proto_turf_for_test).to receive(:encode).and_return('encoded-key')

      encoded = key_backend.encode(key_msg, topic: 'my-topic', is_key: true)

      expect(key_proto_turf_for_test).to have_received(:encode).with(key_msg, subject: 'my-topic-key')
      expect(encoded).to eq('encoded-key')
    end
    ...
    it 'should encode a hash key using the key schema' do
      ...
      expect(key_proto_turf_for_test).to have_received(:encode) do |arg, **kwargs|
    ...
    it 'should decode a protobuf key' do
      key_msg = Sample::V1::SampleMessageKey.new(str: 'test-key')
      allow(proto_turf_for_test).to receive(:decode).and_return(key_msg)

      decoded = key_backend.decode('encoded-key')

      expect(proto_turf_for_test).to have_received(:decode).with('encoded-key')
  ...
  describe 'backward compatibility with plain string/JSON keys' do
    ...
    it 'should encode hash keys using JSON schema when no field specified' do
      ...
      allow(key_proto_turf).to receive(:encode).and_return('encoded-json')

      backend.encode_key(nil, key_hash, topic: 'my-topic')

      # Should use encode_proto_key which calls key_proto_turf.encode
      expect(key_proto_turf).to have_received(:encode)
    end

    it 'should decode JSON string keys to hashes' do
      decoded_hash = { 'foo' => 'bar' }
      allow(key_proto_turf).to receive(:decode).and_return(decoded_hash)
  ...
  describe 'edge cases' do
    ...
    it 'should handle encoding payloads with subject suffix detection' do
      allow(proto_turf).to receive(:encode).and_return('encoded-payload')
      allow(key_proto_turf).to receive(:encode).and_return('encoded-payload')

      # When subject ends with '-key', should use key_proto_turf
      backend.encode_payload(payload, subject: 'my-topic-key')
      expect(key_proto_turf).to have_received(:encode).with(payload, subject: 'my-topic-key')

      # When subject doesn't end with '-key', should use proto_turf
      backend.encode_payload(payload, subject: 'my-topic-value')
      expect(proto_turf).to have_received(:encode).with(payload, subject: 'my-topic-value')
    end

    it 'should handle nil payloads gracefully' do
      # encode with nil payload - proto_turf will handle the nil
      allow(proto_turf).to receive(:encode).with(nil, subject: 'topic-value').and_return(nil)
