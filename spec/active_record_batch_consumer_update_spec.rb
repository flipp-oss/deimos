...
module ActiveRecordBatchConsumerTest
  describe Deimos::ActiveRecordConsumer,
    ...
    context 'with one-to-many relationship in association_list and default bulk_import_id' do
      let(:consumer_class) do
        Class.new(described_class) do
          ...
          def build_records(messages)
            ...
            messages.map do |m|
              payload = m.payload
              w = Widget.new(id: payload['id'],
                             test_id: payload['test_id'],
                             some_int: payload['some_int'],
                             bulk_import_id: bulk_import_id)
              w.locales << Locale.new(title: payload['title'], language: 'en', bulk_import_id: bulk_import_id)
              w.locales << Locale.new(title: payload['title'], language: 'fr', bulk_import_id: bulk_import_id)
              w
            end
          end
      ...
      it 'should save and update items to widget and associated detail' do
        stub_const('MyBatchConsumer', consumer_class)
