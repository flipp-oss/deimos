...
| fallback_to_individual_updates | true        | When a bulk database write fails, retry it one record at a time so that a single record which cannot be persisted doesn't lose the whole batch, then raise `Deimos::BatchFallbackError` naming the keys that still failed. Only the write is retried - `pre_process` and the `valid_records`/`invalid_records` events still happen once per group. Set to false to keep the previous all-or-nothing behaviour, which may be preferable for very large batches on hot topics where retrying row by row is too slow. |
