  ...
  module Config
    class << self
      ...
      def configure_defaults(config)
        ...
        configs["socket.connection.setup.timeout.ms"] = config.kafka.connect_timeout
        configs["socket.timeout.ms"] = config.kafka.socket_timeout
        ...
        configs["request.required_acks"] = config.producers.required_acks
        configs["message.send.max.retries"] = config.producers.max_retries
        configs["retry.backoff.ms"] = config.producers.retry_backoff
      ...
      def configure_consumers(config)
        config.consumer_objects.each do |consumer|
          ...
          configs["auto.offset.reset"] = consumer.start_from_beginning
          configs["session.timeout.ms"] = consumer.session_timeout ? consumer.session_timeout * 1000 : nil
          configs["auto.commit.interval.ms"] = consumer.offset_commit_interval ? consumer.offset_commit_interval * 1000 : nil
          configs["heartbeat.interval.ms"] = consumer.heartbeat_interval ? consumer.heartbeat_interval * 1000 : nil

          Karafka::App.routes.draw do
            topic consumer.topic do
              consumer(consumer.class_name.constantize)
